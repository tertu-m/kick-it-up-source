...
double				Data_Double_y[MAX_DATA+1];//ﾁｷｺｸ ｵ･ﾀﾌﾅﾍﾀﾇ Yｰｪﾀｻ ｰ｡ﾁ・ﾀﾖｴﾂ ｹ霑ｭ
// ｴ・ｵ･ﾀﾌﾅﾍ ｳ｡
...
LPDIRECTDRAWSURFACE	CFont			= NULL;
...
void	DisplayStageCount(DWORD Count)
{
	...
	//g_pDDSBack->BltFast(0, 0, StageCount, NULL, DDBLTFAST_SRCCOLORKEY);
}
...
void KIU_STAGE(void)
{
	...
	DisplayMessageC(0,0,s);
	sprintf(s, "start2: %d", start2);
	DisplayMessageC(0,16,s);
	sprintf(s, "start3: %d", start3);
	DisplayMessageC(0,32,s);
	sprintf(s, "bpm 1:%g", bpm);
	DisplayMessageC(0,48,s);
	sprintf(s, "bpm 2:%g", bpm2);
	DisplayMessageC(0,64,s);
	sprintf(s, "bpm 3:%g", bpm3);
	DisplayMessageC(0,80,s);
	sprintf(s, "bunki :%ld", bunki);
	DisplayMessageC(0,96,s);
	sprintf(s, "bunki2:%ld", bunki2);
	DisplayMessageC(0,112,s);
	...
	if(start1==0)
	{
		if(b4dMix1p==TRUE)
		{
			...
			if(HighSpeed1p_3 > MaxSpeed)MaxSpeed=HighSpeed1p_3;
			if(HighSpeed1p_5 > MaxSpeed)MaxSpeed=HighSpeed1p_5;
			if(HighSpeed1p_7 > MaxSpeed)MaxSpeed=HighSpeed1p_7;
			if(HighSpeed1p_9 > MaxSpeed)MaxSpeed=HighSpeed1p_9;

			if(HighSpeed1p_3 < MinSpeed)MinSpeed=HighSpeed1p_3;
			if(HighSpeed1p_5 < MinSpeed)MinSpeed=HighSpeed1p_5;
			if(HighSpeed1p_7 < MinSpeed)MinSpeed=HighSpeed1p_7;
			if(HighSpeed1p_9 < MinSpeed)MinSpeed=HighSpeed1p_9;
		}
		else 
		{
		...
		if(b4dMix2p)
		{
			...
			if(HighSpeed2p_3 > MaxSpeed)MaxSpeed=HighSpeed2p_3;
			if(HighSpeed2p_5 > MaxSpeed)MaxSpeed=HighSpeed2p_5;
			if(HighSpeed2p_7 > MaxSpeed)MaxSpeed=HighSpeed2p_7;
			if(HighSpeed2p_9 > MaxSpeed)MaxSpeed=HighSpeed2p_9;

			if(HighSpeed2p_3 < MinSpeed)MinSpeed=HighSpeed2p_3;
			if(HighSpeed2p_5 < MinSpeed)MinSpeed=HighSpeed2p_5;
			if(HighSpeed2p_7 < MinSpeed)MinSpeed=HighSpeed2p_7;
			if(HighSpeed2p_9 < MinSpeed)MinSpeed=HighSpeed2p_9;
		}
		else 
		{
	...
	if(Start1p)DrawArrow1p(i); //ﾈｸｻ・ﾈｭｻ・･ｸｦ ｸｻﾇﾕｴﾏｴﾙ.
	...
	if(start<=0)
	{
		...
		//1000 Tickｴ・180/60 -> 1ﾃﾊｿ｡ 64*(180/60)  ﾁ・1 tick ｴ・64*(bpm/60)/1000
...
void DisplayMessageC(int x, int y,char *message)
{
#define FONT_SIZE	8
...
#define FONT_SIZE	8
	...
	for(i=0;;i++)
	{
		...
		g_pDDSPrimary->BltFast(x,y,SmallFont,&sRect, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
		g_pDDSBack->BltFast(x,y,SmallFont,&sRect, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
		x+=FONT_SIZE;
...
void StageTitle(void)
{
	...
	if(Start1p || Start2p)
	{
		if(timeGetTime()-sec>2000)
		{
			sec=timeGetTime();
			PressedKey2p[0]=3;
		}
		else if(Start1p && Start2p)PressedKey2p[0]=3;
		else if(Start1p)
		{
			if(c==0)
			{
				if(PressedKey1p[5]==TRUE)c++;
			}
			else if(c==1)
			{
				if(PressedKey1p[5]==TRUE)PressedKey2p[0]=3;
			}
		}
		else if(Start2p)
		{
			if(c==0)
			{
				if(PressedKey2p[5]==TRUE)c++;
			}
			else if(c==1)
			{
				if(PressedKey2p[5]==TRUE)PressedKey2p[0]=3;
			}
		}
	}
...
void Flipp(void)
{
	...
	while(TRUE)
	{
		if(fullscreen)hRet=g_pDDSPrimary->Flip(NULL,DDFLIP_WAIT);
		else hRet = g_pDDSPrimary->Blt(&g_rcScreen, g_pDDSBack,
...
/*	for(i=0;;i++)
	{
		if(CSONG[i].bpm!=0)
		{
			//if(DEBUGFILE!=NULL)fprintf(DEBUGFILE," %d : %s\n", i, CSONG[i].WavsName);
			//CSONG[i].Int_Song = DSLoadSoundBuffer(lpds,CSONG[i].WavsName);
...
{
	// FPS count start
	static DWORD cur, deltaTime, lastTime, fpsTime,framesRendered,fps;
	...
	switch (message)
    {
            ...
            switch (wParam)
            {
				case VK_F12:
					PostMessage(hWnd, WM_CLOSE, 0, 0);
					return 0L;
/* SelectSong, StageTitleｿ｡ｼｭ ﾂ・ｶﾇﾕｴﾏｴﾙ.
   ﾃﾟﾈﾄｿ｡ ｹﾙｲ翆ﾖｾ﨨ﾟ ﾇﾕｴﾏｴﾙ.
   */
				case VK_F2:
					First=0;
					switch(ProgramState)
...
HRESULT KLoadImage(void)
{
	...
	CFont=DDLoadBitmap(g_pDD, "images\\CFont.bmp",0,0);
	if(CFont==NULL)
	{
		MessageBox(hWnd,"Cannot Load CFont.bmp",0,0);
		return	FALSE;
	}
	DDSetColorKey(CFont, CLR_INVALID);
	CKey_CFont=DDColorMatch(CFont,CLR_INVALID);
...
// Debug File name definition
	...
	sprintf(szDebugName,"%d%d%d.txt",today->tm_year+1900, today->tm_mon+1, today->tm_mday);
// define end :)
