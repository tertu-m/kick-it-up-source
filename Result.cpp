...
//	if(((double)(cPerfect1p+cGreat1p)/(double)cTotal1p)*100)>70)return	'D';
...
//	if(((double)(cPerfect2p+cGreat2p)/(double)cTotal2p)*100)>70)return	'D';
...
void Result(void)
{
	...
	if(Phase==7)
	{
		...
		if(i==60)
		{
			...
			if(dwGameCount==3 || (Judge1p=='F' && Judge2p=='F') )ProgramState=GAMEOVER,dwGameCount=0;
			else
			{
				dwGameCount++;
